...
function CommentsController({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setCanComment(allowed || false);
      setCanSeeComments(allowed || false);

      if (!allowed) return;

      const { comments: fetchedComments } = await getComments(
        apiUrl,
        repoOwner,
        repoName,
        repoID,
      ...
      setComments(reanchorComments(fetchedComments || [], contentRef.current));
    })();
  }, [user]);
